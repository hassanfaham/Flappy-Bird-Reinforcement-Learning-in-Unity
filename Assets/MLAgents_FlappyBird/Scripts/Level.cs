...
public class Level : MonoBehaviour {
    ...
    private PipeComplete GetPipeCompleteWithPipe(Pipe pipe) {
        if (pipeCompleteList != null && pipeCompleteList.Count > 0) {
    ...
    public PipeComplete GetNextPipeComplete() {
        ...
        if (pipeList != null && pipeList.Count > 0) {
            for (int i = 0; i < pipeList.Count; i++) {
                Pipe pipe = pipeList[i];
                if (pipe != null && pipe.pipeBodyTransform != null && pipe.GetXPosition() > BIRD_X_POSITION && pipe.IsBottom()) {
